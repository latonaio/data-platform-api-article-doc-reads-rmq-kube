...
func (c *DPFMAPICaller) HeaderDoc(
	...
	if input.HeaderDoc.Article != nil && *input.HeaderDoc.Article != 0 {
		where = fmt.Sprintf("%s\nAND Article = %d", where, *input.HeaderDoc.Article)
	}
	...
	groupBy := "\nGROUP BY Article, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    Article, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_article_header_doc_data
		` + where + groupBy + `;`)
	if err != nil {
